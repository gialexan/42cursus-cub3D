...
/*   Updated: 2023/08/08 12:33:03 by gialexan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static float	find_x_horz_intersection(float player_y,
					float player_x, float y_intercept, float angle);
	...
	t_intersection *horz, float angle)
{
	...
	horz->x_intercept = find_x_horz_intersection(cub3d->player.y,
			cub3d->player.x, horz->y_intercept, angle);
...
static float	calculate_horz_x_step_increment(float angle)
{
	...
	if ((is_raydir_left(angle) && x_step > 0)
		|| (is_raydir_right(angle) && x_step < 0))
		x_step *= -1;
	return (x_step);
}
