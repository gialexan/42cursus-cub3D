...
/*   Updated: 2023/08/08 12:38:54 by gialexan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static float	find_y_vert_intersection(float player_x,
					float player_y, float x_intercept, float angle);
...
void	vertical_intersection(t_cub3d *cub3d, t_intersection *vert, float angle)
{
	...
	vert->y_intercept = find_y_vert_intersection(cub3d->player.x,
			cub3d->player.y, vert->x_intercept, angle);
...
static float	calculate_vert_y_step_increment(float angle)
{
	...
	if ((is_raydir_up(angle) && y_step > 0)
		|| (is_raydir_down(angle) && y_step < 0))
		y_step *= -1;
	return (y_step);
}
